...
(require "ast.rkt" "gc.rkt" "types.rkt" "utils.rkt" a86/ast)
  ...
  (match p
    ...
    ['zero?
     (seq (assert-integer rax)
          (eq-value 0))]
    ['char?
     (type-pred rax mask-char type-char)]
    ['char->integer
     (seq (assert-char rax)
          (Sar rax char-shift)
          (Sal rax int-shift))]
    ...
    ['box
        (seq
          ; check if there is enough space on heap, if not then do gc
          ; if there isn't enough space after gc, then raise a runtime error
          (compile-box-cons-gc-check 1)
    ...
    ['empty? (eq-value '())]
    ['box?
     (type-pred rax ptr-mask type-box)]
    ['cons?
     (type-pred rax ptr-mask type-cons)]
    ['vector?
     (type-pred rax ptr-mask type-vect)]
    ['string?
     (type-pred rax ptr-mask type-str)]
    ['vector-length
     (let ((zero (gensym))
           (done (gensym)))
    ...
    ['make-vector
     (let ((loop (gensym))
       ...
       (seq (Pop r8)
            ...
            (Sar r8 int-shift)  ; length of vector
            (Mov (Offset rbx 0) r8)
            (Add rbx 8)
